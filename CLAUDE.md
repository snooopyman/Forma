...
**Cobertura del patrón:** 15 de 17 features tienen ViewModel + Interactor (`BodyChartsView` y `PostWorkoutSummaryView` son la excepción — vistas de solo lectura sobre datos ya cargados por su padre). Las 15 ViewModels llaman **exclusivamente** a su Interactor — nunca a un repositorio/servicio directamente (verificado 2026-07-01: se encontraron y corrigieron 11 ViewModels que bypaseaban su Interactor pese a que el triplete Interactor+Protocol+Mock ya existía). De esas 15, 5 añaden además `ViewModelProtocol` + `MockViewModel`: las 4 pantallas raíz de cada tab (Dashboard, MesocycleList, PlanOverview, ProgressOverview) — construidas una vez en `MainTabView` e inyectadas vía `@Entry`/`@Environment` — y `ActiveSession`, que tiene `ViewModelProtocol`+`MockViewModel` pero NO usa `@Entry`/`@Environment`: la View construye `ActiveSessionViewModel` directamente vía `@State` en su `init`. El resto de pantallas (CreatePlan, EditPlan, FoodBrowser, MealDetail, Onboarding, NewMeasurement, PhotoGallery, EditProfile, Settings, MesocycleDetail, WorkoutDay) construyen su Interactor y su ViewModel directamente en el `init` de la View, sin protocolo de ViewModel.
